 ...
 */
int hipnuc_decode(hipnuc_raw_t *raw) { return decode_hipnuc_internal(raw); }
...
int hipnuc_dump_packet(hipnuc_raw_t *raw, char *buf, size_t buf_size) {
  ...
  if (raw->hi91.tag == HIPNUC_ID_HI91) {
    ...
    ret = snprintf(buf + written, buf_size - written,
                   ...
                   "  \"air_pressure\": %.1f\n"
                   "}\n",
                   raw->hi91.main_status, raw->hi91.system_time, raw->hi91.acc[0] * GRAVITY, raw->hi91.acc[1] * GRAVITY,
                   raw->hi91.acc[2] * GRAVITY, raw->hi91.gyr[0], raw->hi91.gyr[1], raw->hi91.gyr[2], raw->hi91.mag[0],
                   raw->hi91.mag[1], raw->hi91.mag[2], raw->hi91.pitch, raw->hi91.roll, raw->hi91.yaw,
                   raw->hi91.quat[0], raw->hi91.quat[1], raw->hi91.quat[2], raw->hi91.quat[3], raw->hi91.air_pressure);
  }
  ...
  /* dump 0x81 packet */
    ...
    ret = snprintf(buf + written, buf_size - written,
                   ...
                   "  \"acc_enu\": [%.2f, %.2f, %.2f],\n"
                   "}\n",
                   raw->hi81.main_status, raw->hi81.ins_status, raw->hi81.gpst_wn, raw->hi81.gpst_tow,
                   raw->hi81.gyr_b[0] * (0.001 * R2D), raw->hi81.gyr_b[1] * (0.001 * R2D),
                   raw->hi81.gyr_b[2] * (0.001 * R2D), raw->hi81.acc_b[0] * 0.0048828, raw->hi81.acc_b[1] * 0.0048828,
                   raw->hi81.acc_b[2] * 0.0048828, raw->hi81.mag_b[0] * 0.030517, raw->hi81.mag_b[1] * 0.030517,
                   raw->hi81.mag_b[2] * 0.030517, (float)raw->hi81.air_pressure, raw->hi81.temperature,
                   raw->hi81.utc_year, raw->hi81.utc_month, raw->hi81.utc_day, raw->hi81.utc_hour, raw->hi81.utc_min,
                   raw->hi81.utc_msec / 1000, raw->hi81.utc_msec % 1000, raw->hi81.pitch * 0.01, raw->hi81.roll * 0.01,
                   raw->hi81.yaw * 0.01, raw->hi81.quat[0] * 0.0001, raw->hi81.quat[1] * 0.0001,
                   raw->hi81.quat[2] * 0.0001, raw->hi81.quat[3] * 0.0001, raw->hi81.ins_lat * 1e-7,
                   raw->hi81.ins_lon * 1e-7, raw->hi81.ins_msl * 1e-3, raw->hi81.pdop * 0.1, raw->hi81.hdop * 0.1,
                   raw->hi81.solq_pos, raw->hi81.nv_pos, raw->hi81.solq_heading, raw->hi81.nv_heading,
                   raw->hi81.diff_age, raw->hi81.undulation * 0.01, raw->hi81.vel_enu[0] * 0.01,
                   raw->hi81.vel_enu[1] * 0.01, raw->hi81.vel_enu[2] * 0.01, raw->hi81.acc_enu[0] * 0.0048828,
                   raw->hi81.acc_enu[1] * 0.0048828, raw->hi81.acc_enu[2] * 0.0048828);
  }
    ...
    ret = snprintf(buf + written, buf_size - written,
                   ...
                   "  \"ins_status\": %u,\n"
                   "  \"data_bitmap\": %u\n",
                   raw->hi83.main_status, (unsigned)raw->hi83.ins_status, (unsigned)raw->hi83.data_bitmap);
    ...
    if (raw->hi83.data_bitmap & HI83_BMAP_QUAT) {
      ret = snprintf(buf + written, buf_size - written, "  ,\"quat\": [%.3f, %.3f, %.3f, %.3f]\n", raw->hi83.quat[0],
                     raw->hi83.quat[1], raw->hi83.quat[2], raw->hi83.quat[3]);
      if (ret > 0) written += ret;
    }
    ...
    if (raw->hi83.data_bitmap & HI83_BMAP_UTC) {
      ret = snprintf(buf + written, buf_size - written, "  ,\"utc\": \"20%02u-%02u-%02u %02u:%02u:%02u.%03u\"\n",
                     (unsigned)raw->hi83.utc.year, (unsigned)raw->hi83.utc.month, (unsigned)raw->hi83.utc.day,
                     (unsigned)raw->hi83.utc.hour, (unsigned)raw->hi83.utc.min, (unsigned)(raw->hi83.utc.sec_ms / 1000),
    ...
    if (raw->hi83.data_bitmap & HI83_BMAP_GNSS_QUALITY_NV) {
      ret = snprintf(buf + written, buf_size - written,
                     "  ,\"solq_pos\": %u\n  ,\"nv_pos\": %u\n  ,\"solq_heading\": %u\n  ,\"nv_heading\": %u\n",
                     (unsigned)raw->hi83.solq_pos, (unsigned)raw->hi83.nv_pos, (unsigned)raw->hi83.solq_heading,
    ...
    if (raw->hi83.data_bitmap & HI83_BMAP_GNSS_VEL) {
      ret = snprintf(buf + written, buf_size - written, "  ,\"gnss_vel\": [%.3f, %.3f, %.3f]\n", raw->hi83.gnss_vel[0],
                     raw->hi83.gnss_vel[1], raw->hi83.gnss_vel[2]);
      if (ret > 0) written += ret;
    }
